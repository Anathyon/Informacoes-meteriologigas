export const relogio = (p_lo_hrs) => {
    ...
    const arelogio = () => {
        ...
        const formatarNumero = (num) => (num < 10 ? "0" + num : num.toString());
...
export const func_dialog = () => {
    const bt_abre = document.querySelector("#abre");
    const bt_fecha = document.querySelector("#fecha");
    const cx_historico = document.querySelector("#cx_historico");
...
const armazena = document.querySelector("#armazena");
export const armazena_historico = (local) => {
    if (!armazena) {
...
export const bt_remove_selecionado = () => {
    const remo_selecionado = document.querySelector("#remo_selecionado");
    const armazena = document.querySelector("#armazena");
...
cx_historico.addEventListener("keydown", (e) => {
    e.preventDefault();
    const elementos = Array.from(armazena.querySelectorAll(".list_p_locais"));
    ...
    if (e.key === "ArrowDown") {
        ...
        const selecionados = elementos.filter((el) => el.classList.contains("selecao"));
            ...
            selecionados.forEach((elemento) => {
                ...
                if (texto) {
                    try {
                        ...
                        const verificaMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
                        alert(`Texto copiado para a área de transferência! ${verificaMobile ? "(Mobile)" : ""}`);
                    }
                    catch (err) {
                        console.error("Erro ao copiar o texto: ", err);
                        alert("Erro ao copiar o texto!");
